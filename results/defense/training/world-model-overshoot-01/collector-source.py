...
destination = Path('results/defense/training/world-model-overshoot-01/collector-config.json')
if '--prepare' in sys.argv:
    old = json.loads(Path('results/defense/training/dqn-58-temporal-consistency/collector-config.json').read_text())
    sources = old['sources'] + ['runs/'+name+'/artifacts' for name in (
        'defense-imagination-calibration-01', 'defense-imagination-uniform-02',
        'defense-imagination-boundary-03', 'defense-imagination-feedback-04',
        'defense-imagination-feedback-05', 'defense-imagination-overshoot-control-06',
        'defense-imagination-overshoot-four-07')]
    ...
    config = dict(sources=sources, output=old['output'], run=old['run'], interval=old['interval'],
        previous_pid=70955, previous_sources_preserved=True,
