"""Retain all prior replay sources, including both complete-boot searches."""
...
if '--prepare' in sys.argv:
    old = json.loads(Path('results/defense/training/ars-score-gated-70/collector-config.json').read_text())
    sources = old['sources'] + [
        'runs/defense-ars-boot-pilot-71/artifacts',
        'runs/defense-ars-boot-72/artifacts',
    ]
    ...
    destination.write_text(json.dumps(dict(sources=sources, output=old['output'],
        ...
        new_sources='complete-boot score search from repeated barrier'), indent=2)+'\n')
