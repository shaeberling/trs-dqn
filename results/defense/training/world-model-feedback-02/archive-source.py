import hashlib
import json
from pathlib import Path
import subprocess

def sha(p): return hashlib.sha256(p.read_bytes()).hexdigest()
root = Path('results/defense/training/world-model-feedback-02')
root.mkdir(exist_ok=False)
run = Path('runs/defense-world-fit-07-actor-feedback')
events = [json.loads(l) for l in (run/'metrics.jsonl').read_text().splitlines()]
assert events[-1]['event'] == 'finished' and events[-1]['updates'] == 16000
hashes = {}
for source, dest in [(run, root/'fit'), (Path('runs/defense-world-data-feedback-02'), root/'data')]:
    subprocess.run(['cp', '-cR', str(source), str(dest)], check=True)
    for path in source.rglob('*'):
        if path.is_file():
            digest = sha(path)
            assert sha(dest/path.relative_to(source)) == digest
            hashes[str((dest/path.relative_to(source)).relative_to(root))] = digest
for checkpoint in (root/'fit').glob('update-*'):
    state = json.loads((checkpoint/'state.json').read_text())
    for name, digest in state['hashes'].items(): assert sha(checkpoint/name) == digest
manifest = json.loads((root/'data/manifest.json').read_text())
assert len(manifest['episodes']) == 96 and manifest['heldout_games'] == 16
for row in manifest['episodes']: assert sha(root/'data'/row['file']) == row['sha256']
(root/'archive-audit.json').write_text(json.dumps(dict(file_hashes=hashes, dynamics_updates=16000,
    new_dynamics_updates=2000, no_evaluation_training_data=True), indent=2)+'\n')
print(json.dumps(dict(archived_files=len(hashes), dynamics_updates=16000)))
