"""Retain all preceding policy sources, including both focused calibrations."""
...
if '--prepare' in sys.argv:
    old = json.loads(Path('results/defense/training/ars-focus-61/collector-config.json').read_text())
    sources = old['sources'] + [
        'runs/defense-ars-focus-62-wide-search/artifacts',
        'runs/defense-ars-focus-63-bias-search/artifacts',
    ]
    ...
    destination.write_text(json.dumps(dict(sources=sources, output=old['output'],
        ...
        new_sources='wide-head and action-bias focused neural searches'), indent=2)+'\n')
