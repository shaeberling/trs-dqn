"""Retain prior replay candidates, including the current coordinate search."""
...
if '--prepare' in sys.argv:
    old = json.loads(Path('results/defense/training/ars-focus-63/collector-config.json').read_text())
    sources = old['sources'] + [
        'runs/defense-ars-focus-64-early-bias/artifacts',
        'runs/defense-ars-focus-65-coordinate/artifacts',
    ]
    ...
    destination.write_text(json.dumps(dict(sources=sources, output=old['output'],
        ...
        new_sources='earlier-own-state and symmetric coordinate searches'), indent=2)+'\n')
