...
root = Path('results/defense/training/world-model-boundary-01')
root.mkdir(exist_ok=True)
summary = {}
for arm, run_name in [('uniform', 'defense-world-fit-04-uniform-control'),
                      ('focused', 'defense-world-fit-05-boundary-sampling')]:
    source = Path('runs')/run_name
    events = [json.loads(line) for line in (source/'metrics.jsonl').read_text().splitlines()]
    assert events[-1]['event'] == 'finished' and events[-1]['updates'] == 12000
    dest = root/arm
    ...
    origin = read(source/'update-010000/state.json')
    final = read(source/'update-012000/state.json')
    audit = read(source/'update-012000/audit.json')
    boundaries = read(source/'update-012000/boundary-predictions.json')
...
for name in ('optimizer.npz', 'random.npz'):
    with np.load(root/'uniform/update-010000'/name) as a, np.load(root/'focused/update-010000'/name) as b:
...
for arm in ('uniform', 'focused'):
    assert read(root/arm/'update-010000/state.json')['sampling_rng'] == read(root/'uniform/update-010000/state.json')['sampling_rng']
summary['limitations'] = ['Loss-window sampling intentionally changes the training distribution; no unbiased-calibration claim.',
