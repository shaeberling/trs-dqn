...
source = Path('runs/defense-imagination-feedback-04')
dest = Path('results/defense/training/imagination-feedback-01')
...
assert events[-1]['actor_updates'] == 1500 and not dest.exists()
...
parent = Path('results/defense/training/imagination-boundary-comparison-01/uniform/update-000500')
start = dest/'update-000500'
...
result = dict(file_hashes=hashes, evaluations=evaluations, verified_actions=verified['verified_actions'],
    ...
    old_world_changed=True, parent_actor_updates=500, additional_actor_updates=1000,
