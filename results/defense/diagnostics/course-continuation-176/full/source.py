...
def run(bundle, discovery, output, *, beam_width=512, target_rows=75,
    ...
    try:
        ...
        with (output / "layers.jsonl").open("x", buffering=1) as stream:
            for frame in range(SOURCE_FRAME + 1, max_frame + 1):
                ...
                if not discovery_result:
                    ...
                    if current:
                        # Record a witness only after selection. This does not
                        # alter the beam, RNG, ranking or target-stop decision.
        ...
        if discovery_result is None and frontier_node is not None:
            ...
            full_path = np.concatenate((source_actions,
                                        np.frombuffer(frontier_node.path, np.uint8)))
            expected = dict(frame=frontier_frame, rows=rows, stage=env.stage,
