...
groups={name:dict(cells=0,correct=0,squared_error=0.,persistence_correct=0) for name in ('nonblank_ascii','changed_ascii','digits','stars','changed_hud_cells')}
...
report=dict(checkpoint_sha256=sha256(checkpoint/'world.safetensors'),dataset_sha256=sha256(data/'manifest.json'),
    source_sha256=sha256(Path(__file__)),groups=groups,windows=64,post_burn_arrival_frames=1536,
...
root=Path('results/defense/diagnostics/world-text-reconstruction-01');root.mkdir(parents=True,exist_ok=False)
