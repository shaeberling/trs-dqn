...
def hold_lengths(mode):
    if mode == 'short':
        return HOLDS
    if mode == 'long':
        return LONG_HOLDS
    raise ValueError('unknown training-only hold schedule')


def draw_plan(rng, mode, lookback=128, hold_mode='short'):
    commands = command_ids(mode)
    branch = int(rng.choice(branches_for_lookback(lookback)))
    segments = [(int(rng.choice(commands)), int(rng.choice(hold_lengths(hold_mode))))
                for _ in range(2)]
    return branch, segments


def play_trial(env, saved, recorded_actions, recorded_rewards, recorded_screens,
               policy, seed, branch, segments, *, max_actions, holds=HOLDS,
               keep_trace=False):
...
def main():
    ...
    parser.add_argument('--hold-mode', choices=('short', 'long'), default='short')
    ...
    settings = dict(checkpoint=str(args.checkpoint.resolve()),
                    ...
                    branches=list(branches_for_lookback(args.lookback)),
                    holds=list(hold_lengths(args.hold_mode)),
    ...
    with (args.output/'metrics.jsonl').open('x') as stream:
        ...
        try:
            for attempt in range(args.attempts):
                ...
                branch, segments = draw_plan(rng, args.command_mode,
                                             args.lookback, args.hold_mode)
                ...
                result, _ = play_trial(env, saved, actions, rewards, screens,
                    ...
                    holds=hold_lengths(args.hold_mode))
                ...
                if result['completed_boundary'] and result['highest_stage'] > 1:
                    verified, trace = play_trial(env, saved, actions, rewards, screens,
                        ...
                        max_actions=args.max_actions, holds=hold_lengths(args.hold_mode),
                        keep_trace=True)
                    if verified != result:
