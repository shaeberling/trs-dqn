...
def run(bundle, output, *, beam_width=128, layers=0, seed=172,
        verification_goal_frame=SURVIVAL_FRAME):
    ...
    if verification_goal_frame not in numbered or verification_goal_frame <= TARGET_FRAME:
        raise ValueError("verification goal must be a scheduled frame after first-gate approach")
    ...
    config = dict(bundle=str(bundle.resolve()), model_sha256=sha256(bundle / "model.safetensors"),
                  ...
                  verification_goal_frame=verification_goal_frame,
    ...
    try:
            ...
            for layer, duration in enumerate(SCHEDULE[:requested], 1):
                ...
                for parent in current:
                    for action in SIDE_FIRE_COMMANDS:
                        ...
                        if (info["stage"] >= 2 or info["missions_completed"]
                                or numbered[layer] >= verification_goal_frame):
        ...
        result = dict(config=config, completed_layers=last_row["layer"] if last_row else 0,
                      ...
                      limitations=["A finite beam from one verified first life can miss passable routes.",
                                   ...
                                   "Survival to a diagnostic goal frame is not a stage clear or successful mission."])
...
def main():
    ...
    parser.add_argument("--verification-goal-frame", type=int, default=SURVIVAL_FRAME,
                        help="scheduled survivor frame; default 428")
    args = parser.parse_args()
    result = run(args.bundle, args.output, beam_width=args.beam,
                 layers=args.layers, seed=args.seed,
                 verification_goal_frame=args.verification_goal_frame)
